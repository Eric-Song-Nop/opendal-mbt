...
native or browser implementation layout; those details must implement this
contract without leaking through the public package.

This document defines the implemented resource, S3, presign, layer, batch,
Copier, portable async, and Browser JS semantics. Public reference is
target-specific:

| Target | Authoritative view |
| --- | --- |
| Native | `src/pkg.generated.mbti`, generated by `moon info --target native` |
| JavaScript | `moon ide doc --target js '@Eric-Song-Nop/opendal/browser'` and the compiler-checked public declarations in `src/browser` |

A single committed `.mbti` cannot describe both conditional surfaces. The
[browser API reference](../reference/browser-api.md) records their portable
intersection and JavaScript extensions. Supported public targets are native
and JavaScript; this contract does not promise `wasm`, `wasm-gc`, or another
MoonBit backend. The Rust WebAssembly bridge embedded by the JavaScript target
is an internal implementation detail, not a MoonBit Wasm target.
...
|---|---|---|
| Native blocking resources | `Operator`, `Lister`, `Reader`, `ReadStream`, `Writer`, `Copier` | Opaque native handles; impossible for callers to fabricate |
| Portable async resources | `AsyncOperator`, `AsyncReadStream`, `AsyncWriter` | Same public slice on native and JavaScript; target-specific ownership behind it |
| JavaScript runtime resources | `Runtime`, `Operator`, `AsyncLister`, `BrowserAssets` | One browser Wasm runtime, configured operators, and explicit/custom asset loading |
| Native opaque configuration | `S3Auth`, `S3CredentialSource` | Constructed only through typed, secret-safe factory methods |
| Read-only snapshots | `Metadata`, `Entry`, `ErrorInfo`, `OperatorInfo`, `Timestamp`, and native `PresignedRequest`/`PresignedHeader` | Fields are readable but values are produced by the binding |
...
On native, `Operator` is logically immutable and shareable; layer methods
return new Operators, and `AsyncOperator` is a lightweight view of the same
configured operator. Native `Lister`, `ReadStream`, `Writer`, `Copier`,
`AsyncReadStream`, and `AsyncWriter` are stateful. `Reader` is a random-access
reader without an implicit cursor.

On JavaScript, each embedded `Runtime::new` owns one Rust bridge Wasm instance,
and `Operator` retains that runtime. Explicit `Runtime::load` construction is
limited to one call per bridge module URL for the lifetime of a page because
browser ESM and wasm-bindgen cache initialized exports. `AsyncOperator` is a
view of the JavaScript operator;
`AsyncReadStream`, `AsyncWriter`, and `AsyncLister` are stateful. Those
different ownership trees are why parent-close behavior is deliberately
outside the portable contract.

## Native blocking construction and configuration
...
builds pass `--target js`. Other MoonBit Wasm targets are not supported by this
facade. Both supported targets expose the same portable entry point; the
JavaScript implementation lazily initializes its embedded Rust OpenDAL Wasm
...
operator early. Native async methods launch the real OpenDAL async future on a
Rust Tokio task; they never wrap or invoke the blocking `Operator` as async
work. A Tokio worker owns copied inputs and results, then signals readiness
through a dedicated nonblocking completion pipe while the MoonBit scheduler is
suspended on the read end. It never calls MoonBit from a foreign thread. The
JavaScript target instead waits on Promises and connects MoonBit cancellation
through `AbortSignal` to the browser task bridge. Neither target exposes its
task handle or callback machinery publicly.
...
- [Browser runtime and Wasm ABI](browser-runtime.md)
